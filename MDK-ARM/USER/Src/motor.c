...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if(htim->Instance==htim13.Instance)		              //20ms中断
	{
		...
		motor1.speed = (float)(motor1.totalAngle - motor1.lastAngle)/(4*11*RR)*3000;
		motor2.speed = (float)(motor2.totalAngle - motor2.lastAngle)/(4*11*RR)*3000;
		// printf("speed1:%f\r\n",motor1.speed);//发送速度给串口
		// printf("speed2:%f\r\n",motor2.speed);//发送速度给串口	
		printf("cycle:%f\r\n",((float)motor1.totalAngle/(4*11*RR)));//发送速度给串口		
...
void Speed_Tset1(void){//调电机1速度PID
	...
	if(motor1.pid.inner.output > 0*1.0)        //对应正转
	{
